...
class Server(QuicConnectionProtocol):
    ...
    async def reply(self, sid):
        ...
        for i in range(5):
            ...
            self._transport.write(b''.join(d[0] for d in datagrams))
...
async def main():
    ...
    async with connect('127.0.0.1', 4435, configuration=cc, create_protocol=Client) as qc:
        ...
        datagrams = qc._quic.datagrams_to_send(now=time.time())
        qc._transport.write(b''.join(d[0] for d in datagrams))
